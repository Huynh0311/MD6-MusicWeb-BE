...
public interface ILikesRepository extends JpaRepository<Likes,Integer> {
    ...
    @Query(nativeQuery = true,value = "select * from likes l where l.account_id = :idAccount")
    List<Likes> findSongByLike(@Param("idAccount") int idAccount);

    List<Likes> findAllByAccountEmail(String email);
}
